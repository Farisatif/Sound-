...
export const TrendingSongsSection = (): JSX.Element => {
  ...
  return (
    <motion.section
      className="w-full relative px-4 py-8 sm:px-6 lg:px-12"
      initial={{ opacity: 0 }}
    ...
    >
      {/* Section Title */}
      <div className="flex items-center justify-center gap-2.5 p-2.5 mb-6">
      ...
      <div className="space-y-4">
        {songs.trendingSongs.map((song, index) => (
          <motion.div
            ...
            viewport={{ once: true }}
            className="grid grid-cols-1 sm:grid-cols-[auto_1fr_auto] md:grid-cols-[auto_1fr_auto_auto_auto] gap-4 items-center hover:bg-[#1a1a1a] p-3 rounded-lg transition-all duration-200"
          >
            {/* Rank */}
              ...
              <CardContent className="flex items-center gap-4 p-2 h-[60px]">
                <div className="relative group w-16 h-16 flex-shrink-0">
                  <Button
                    ...
                    size="icon"
                    className="absolute inset-0 opacity-0 transition-opacity duration-300 rounded-[5px] w-full h-full z-10"
                  />
                  <motion.img
                    src={song.image}
                    alt="Album cover"
            ...
            <div className="hidden md:block text-white font-text-medium">{song.releaseDate}</div>

            {/* Album (hidden on mobile) */}
            <div className="hidden md:block text-white font-text-medium max-w-[200px] truncate">{song.album}</div>
